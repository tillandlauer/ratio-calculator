...
//
// -----------------------
// Intensity Analysis v1.1
// -----------------------
//
// Till Andlauer (till@andlauer.net)
// Date: 12-10-09 - 13-08-21
//
...
//
// Error propagation according to: Lothar Papula, Mathematik f�r Ingenieure und Naturwissenschaftler Band 3, 5. Auflage 2008, Kapitel 4.3, Seite 678ff: Gau�sches Fehlerfortpflanzungsgesetz
...
public class Intensity_Analysis implements PlugIn
    {   
    private String title = "Intensity Analysis v1.1";
...
//	            ratioData = new double[nChannels][2][data.length]; // mean and sem
        ...
        if (job==3) // Comparative statistics, i.e. compare several situations
            ...
            double[][] data = rio.loadStatFiles(1, 6, true, " Summary 1-1", false); // read 1 line per file, 6 columns per file, inverse format: data = [files][data], add " Summary 1-1" to the input name, don't just get the median but all data
            if (data==null) return false;
            ...
            data = rio.loadStatFiles(2, 6, true, " Summary 1-1", false); // read 2 lines per file, 6 columns per file, inverse format: data = [files][data], add " Summary 3" to the input name, don't just get the median but all data
            if (data==null) return false;
            rt.reset();
            ...
            ArrayList<ArrayList<Double>> dList = rio.loadStatList(" Summary 3", 0); // read 'XX Statistics Summary 4.xls' files 
            if (dList==null) return false;
            Ratio_Statistics rs = new Ratio_Statistics(nFiles, directory, xTitle);
            boolean success = rs.statTestMWU(dList, ""); // do statistical tests and write the file
