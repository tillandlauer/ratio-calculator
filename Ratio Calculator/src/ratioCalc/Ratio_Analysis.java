...
//
// --------------------
// Ratio Analysis v1.36
// --------------------
//
// Till Andlauer (till@andlauer.net)
// Date: 11-10-xx - 15-05-24
//
...
//
// Error propagation according to: Lothar Papula, Mathematik fuer Ingenieure und Naturwissenschaftler Band 3, 5. Auflage 2008, Kapitel 4.3, Seite 678ff: Gausssches Fehlerfortpflanzungsgesetz
...
// Maintenance, compatibility adaptations, added comments, bug fixes (error propagation, normalize files to a reference)
//
// v1.33
// Added Fingerprints.
//
...
// Overhauled the option "Normalize files to a reference".
...
//
// v1.36
// Removed scatter plots.
//
// _____________________________________________________________________________________________________________

public class Ratio_Analysis implements PlugIn
    {   
    private String title = "Ratio Analysis v1.36";
    private String saveError = "Couldn't save file.";
    private boolean all = false; // job=2
    private boolean overview = false; // job=3
 
    // All of the following options can be set in chooseImages():
    private int job = 2; // type of analysis to run; 0 = Histograms, 1 = Statistics, 2 = Histograms and Statistics, 3 = histogram overview, 4 = compare groups, 5 = normalize files to a reference, 6 = fingerprint, 7 = proportions, 8 = Subtract histograms
    ...
    private String[] refFiles; // files for normAnalysis()
    ...
    public void run(String arg) 
        ...
        else if (job==6) success = fingerAnalysis();
        else if (job==7) success = propAnalysis();
        else if (job==8) success = subtractHistos();
        }
    ...
    public boolean basicAnalysis() // do all calculations and create the output files
        ...
        return true;
        } // end of basicAnalysis(), jobs 0,1,2

    
    public boolean compAnalysis() // compare groups, job 4
        {
        TextPanel tp; // content of the output files
        String imagename = ""; // name of the images created by the gnuplot scripts
        String filename = ""; // name of the gnuplot script
        ...
        return true;
        } // end of job 4, compare groups

    
    public boolean normAnalysis() // normalize files to a reference (job 5)
        {
        TextPanel tp; // content of the output files
        String imagename = ""; // name of the images created by the gnuplot scripts
        String filename = ""; // name of the gnuplot script
        ...
        // NOTE: Error propagation according to: Lothar Papula, Mathematik f�r Ingenieure und Naturwissenschaftler Band 3, 5. Auflage 2008, Kapitel 4.3, Seite 678ff: Gau�sches Fehlerfortpflanzungsgesetz
        // Division of two values X,Y with uncertainties sX,sY:        e = SQRT((sX/X)^2 + (sY/Y)^2))
        ...
        ResultsTable rt = ResultsTable.getResultsTable(); // final normalized values (signal, second set, '…S[]')
        rt.reset();
        rt.setPrecision(9);
        ...
        return true; 
        }  // end of job 5, normalize files to a reference


    public boolean fingerAnalysis() // generate fingerprints, job 6
	    {
        if ((nFiles % 2)!=0)
        	{
...
//        	ratio = data[i];
     ...
     * @return the double[]
     */
    private double[] generateBins() // used in fingAnalysis()
        {
    ...
    public boolean propAnalysis() // calculate proportions (relative distribution) - job 7
    {
    Ratio_InOut rio = new Ratio_InOut(nFiles, directory);
    int[][] data = rio.loadHistoFiles("Unbinned Results"); // all histogram files; [bins][files]
    if (data==null) return false;
    int[] count = new int[3];
    ...
    return true;
  	} // end of propAnalysis(), job 7, relative distribution
    

    public boolean subtractHistos() // job 8
    {
    if ((nFiles % 2)!=0)
		{
    ...
    private boolean fileDialog() // ask the user what to do
        {
        String[] purpose_temp = new String[9]; // which analyses to run
        purpose_temp[0] = "Basic Statistics: Mean Histogram";
        purpose_temp[1] = "Basic Statistics: Mean Statistics";
        purpose_temp[2] = "Basic Statistics: Histogram and Statistics";
        purpose_temp[3] = "Basic Statistics: Generate overview of all histograms";
        purpose_temp[4] = "Additional Statistics: Compare groups";
        purpose_temp[5] = "Additional Statistics: Normalize files to a reference";
        purpose_temp[6] = "Additional Statistics: Generate Fingerprint image";
        purpose_temp[7] = "Additional Statistics: Calculate relative distribution (low-balanced-high)";
        purpose_temp[8] = "Additional Statistics: Subtract two histograms";
        ...
        gd.addMessage("This plugin exists for compatibility. Please rather analyze the unbinned Ratio Calculator results using R.");
        gd.addMessage("");
        ...
        if (job==0 || job==1 || job==2 || job==4 || job==5) // if histogram or statistics shall be calculated
            {
        ...
        if (job==3) // only create histogram overview
            {
            job = 0;
            overview = true;
            }
        
        if (job==4) // compare groups
        ...
        if (job==5) // normalize files to a reference
            {
        ...
        if (job==6) // choose file for fingerprint
        	{
        	OpenDialog od;
