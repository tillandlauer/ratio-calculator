...
//
// ------------------------------------
// Mask Generator RGB v1.0
// ------------------------------------
//
// Till Andlauer
// Date: 11-03-01 - 14-07-24
//
// Adaptation of Mask Generator with support for RGB image stacks.
//
...
public class Mask_Generator_RGB implements PlugIn
    {
    private String title = "Mask Generator RGB v1.0"; // title for dialogs and error messages
    ...
    public void run(String arg) 
        ...
        if (img[0].getType() != ImagePlus.COLOR_RGB)
        	{
        	IJ.error("Image type not supported, please use an RGB stack.");
        	return;
        	}             

        if (img[0].getStackSize()==1)
	        {
	    	IJ.error("Single image planes are not supported, please use an RGB stack.");
	    	return;
	        }

        ImageStack tempStack;               
        ImagePlus tempImg;
        // Split RGB channels into 8-bit
        ImageStack stack = img[0].getStack();
        ImageStack[] channels = ChannelSplitter.splitRGB(stack, true);
        IJ.freeMemory();
        String imgTitle = img[0].getTitle();
        
        // Mask image
        ImagePlus[] img_mask = new ImagePlus[3];

        for (int i=0; i<3; i++)
        	{
            img[0] = new ImagePlus(imgTitle, channels[i]);
            img_mask[i] = createMask(img);
            if (img_mask[i] == null) return;
            }

        int w  = img[0].getWidth(); 
        int h = img[0].getHeight();
        int s = img[0].getStackSize();
        if ((!thresh) || mask) // the job is finished then
        	{
	        RGBStackMerge rsm = new RGBStackMerge();
	        ImageStack s_rgb = rsm.mergeStacks(w, h, s, img_mask[0].getStack(), img_mask[1].getStack(), img_mask[2].getStack(), false);
	        ImagePlus mask_rgb = new ImagePlus(imgTitle+" Mask", s_rgb);   
	        mask_rgb.show();
	        
	        if (thresh && mask) // show a copy of the mask without thresholding
		        {
		        ImagePlus img_show = new Duplicator().run(mask_rgb);
		        img_show.setTitle(mask_rgb.getTitle());
		        img_show.show();
		        }            
        	}
        ...
        if (thresh)
            ...
            img[1].close();
            ImagePlus[] img_thresh = new ImagePlus[3];
            ImagePlus thresh_rgb = null;
            if (!manual && (cutOff>=img_mask[0].getNSlices())) tSlice=false; // no substacks if the complete stack contains too few slices
            ...
            if (tSlice) // divide the stack into substacks; find a threshold for each substack
                ...
                for (int k=0; k<3; k++)
	                ...
	                inputStack = img_mask[k].getStack();
	                ...
	                if(size!=eSlice[0]) //channel is not empty
	                	{
	                    IJ.log("Processing channel "+k+":");
		                ...
		                img_thresh[k] = new ImagePlus("Mask", finalStack);
		                img_thresh[k].copyScale(img[k]);
		                ...
		                mean[k] = mean[k] / c;
		                IJ.log("Mean threshold of channel "+k+": "+IJ.d2s(Math.round(mean[k]),0));                
		                IJ.log("");
	                	}
	                else //channel is empty
	                	{
		                img_thresh[k] = img_mask[k];
	                	mean[k] = -99;
		                IJ.log("Channel "+k+" was blank.");                
		                IJ.log("");
	                	}
	                }

                double mMean = 0;
                if (mean[0]==-99 || mean[1]==-99 || mean[2]==-99)
                	{
                    if (mean[0]!=-99 || mean[1]!=-99)
                    	{
                    	mMean = (mean[0]+mean[1])/2;
                    	}
                	}
                else
                	{
                	mMean = (mean[0]+mean[1]+mean[2])/3;
                	}
	            RGBStackMerge rsm = new RGBStackMerge();
		        ImageStack s_rgb = rsm.mergeStacks(w, h, s, img_thresh[0].getStack(), img_thresh[1].getStack(), img_thresh[2].getStack(), false);
		        thresh_rgb = new ImagePlus(imgTitle+" Thresholded", s_rgb);   
                if (mMean!=0) thresh_rgb.setTitle("Mask mean "+IJ.d2s(Math.round(mMean),0));  
                }
            ...
            // Threshold with a value based on the entire stack, not for substacks
                ...
                for (int k=0; k<3; k++)
		        	{
                	img_thresh[k] = execThresh(img_mask[k], true, true);
		        	}
		        RGBStackMerge rsm = new RGBStackMerge();
		        ImageStack s_rgb = rsm.mergeStacks(w, h, s, img_thresh[0].getStack(), img_thresh[1].getStack(), img_thresh[2].getStack(), false);
		        thresh_rgb = new ImagePlus(imgTitle+" Thresholded", s_rgb);   
    ...
    private boolean loadConfig()
		...
		if (rc.error) return false;
			...
			String[] booleans = {"rgb", "dupl", "merge", "thresh", "tSlice", "iThresh", "showRes", "manual", "ratio", "maskM", "keepAM", "perc"};
			int cInt = 0;
			double cDouble = 0.0d;
			boolean cBool = false;
    ...
    private ImagePlus[] chooseImages() // Let the user choose the images to work on.
        ...
        if (open_images.length<2)
        	{
        	IJ.error(title, "At least an image stack and a mask need to be open.");
        	return null;
        	}                                   
        ...
        gd.addChoice("RGB Stack:", image_titles, image_titles[0]);
        ...
        // Process the input
        mask = false;; // important in case open_images.length<2
        ...
        if (logInfo)
        	{
        	IJ.log("Stack: "+img_out[0].getTitle());
        	IJ.log("Mask: "+img_out[1].getTitle());
        	if (thresh) 
        		{
